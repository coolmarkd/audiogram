...
async function transcribe(audioPath, options, callback) {
  ...
  try {
    // Upload and transcribe the audio file with speaker diarization
    var transcript = await client.transcripts.transcribe({
      audio: audioPath,
      speech_model: "best",
      language_code: "en",
      punctuate: true,
      format_text: true,
      speaker_labels: true,
      speakers_expected: 2  // Default to 2 speakers, can be overridden
    });
    ...
    var segments = parseAssemblyAIResponse(transcript, options);
...
function parseAssemblyAIResponse(transcript, options) {
  ...
  transcript.words.forEach(function(word) {
    ...
    var speaker = word.speaker || "Speaker A"; // Default speaker if not provided
    ...
    } else if (wordStart - currentSegment.start < maxDuration && currentSegment.speaker === speaker) {
      // Add to current segment (same speaker, within duration limit)
