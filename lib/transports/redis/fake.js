...
function load() {
  try {
    ...
    if (!content || content.trim() === '') {
      console.log("Job queue file is empty, returning empty object");
      return {};
    }
    return JSON.parse(content);
  } catch (e) {
    console.warn("Error loading job queue file, initializing with empty object:", e.message);
    console.warn("File path:", filename);
    return {};
  }
}
